...
AFRAME.registerGeometry('sequenceFlowWall', {
  schema: {
    points: {
      default: ['0 0', '10 10']
    }
...
function handleSequenceFlow(scene, element) {
  ...
  */
  ...
  flow.setAttribute('isSequenceFlow', 'true');

  const flowWall = document.createElement('a-entity');
  flowWall.setAttribute('geometry', 'primitive: sequenceFlowWall; points:' + points + ';');
  flowWall.setAttribute('material', 'color: #FFFFFF; side: double;');
  flowWall.setAttribute('model-entity', 'true');
  flowWall.setAttribute('isSequenceFlowWall', 'true');
  ...
  line.setAttribute('isSequenceFlowLine', 'true');

  scene.appendChild(flow);
  scene.appendChild(flowWall);
